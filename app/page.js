  ...
  return (
    <div className="bg-[url('/grain-texture.png')] bg-fixed bg-cover bg-white overflow-hidden">
      ...
      <AnimatedSection className="max-w-7xl mx-auto px-4 py-24">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <motion.div variants={fadeIn}>
            ...
            <p className="text-forest-700 mb-4 text-lg">
            ...
            <Link
              href="/about"
              className="inline-flex items-center gap-2 bg-sage-600 text-white px-6 py-3 rounded-xl font-semibold hover:bg-sage-500 transition shadow-md"
            >
              Learn More
      ...
      <section className="bg-sage-50 py-24">
        <div className="max-w-7xl mx-auto px-4">
          <motion.h2
            ...
            viewport={{ once: true }}
            className="text-4xl font-bold text-forest-800 text-center mb-16"
          >
            ...
            {[
              {
                ...
                icon: <GiPlantSeed className="text-sage-600 text-5xl" />,
              },
              {
                title: "Freshness Guaranteed",
                desc: "Prepared & stored with care for uncompromised freshness.",
                icon: <FaLeaf className="text-sage-600 text-5xl" />,
              },
              {
                title: "Trusted Network",
                desc: "Supporting 5,00,000+ farmers with a strong supply chain.",
                icon: <FaHandsHelping className="text-sage-600 text-5xl" />,
              },
            ].map((item, index) => (
              <motion.div
                key={index}
              ...
              >
                ...
                <h3 className="font-bold text-forest-700 text-xl mb-3">
                  {item.title}
                </h3>
                <p className="text-forest-600">{item.desc}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <AnimatedSection className="max-w-7xl mx-auto px-4 py-24">
        <motion.h2
          variants={fadeIn}
          className="text-5xl font-bold text-forest-800 mb-4 text-center"
        >
          Premium Pulses Collection
        </motion.h2>
        <motion.p
          variants={fadeIn}
          className="text-lg text-forest-600 mb-12 text-center max-w-2xl mx-auto"
        >
            ...
            {[
              ...
              <SwiperSlide key={product.id}>
                <motion.div
                  variants={fadeIn}
                  whileHover={{ y: -5 }}
                  className="flex flex-col items-center text-center bg-gradient-to-br from-white to-sage-50 p-6 rounded-3xl shadow-xl border border-sage-200 relative overflow-hidden"
                >
                  {/* Premium tag */}
                  <div className="absolute top-4 right-4 bg-olive-500 text-white text-xs font-bold px-3 py-1 rounded-full z-10">
                    {product.tag}
                  </div>

                  {/* Product image */}
                  <div className="relative w-full h-96 mb-6 flex items-center justify-center"> 
                    <div className="absolute inset-0 bg-gradient-to-br from-sage-100 to-olive-50 rounded-2xl -z-10"></div>
                    <img
                      src={product.image}
                      alt={product.name}
                  ...
                  <p className="text-forest-600 mb-4 text-sm leading-relaxed">
                    {product.description}
                  </p>
                  <ul className="space-y-2 text-sm text-forest-700 w-full">
                    {product.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-3 justify-start bg-sage-50 px-3 py-2 rounded-lg">
        ...
        <div className="hidden md:grid md:grid-cols-3 gap-8">
          {[
            ...
            <motion.div
              ...
              variants={fadeIn}
              whileHover={{ y: -8 }}
              className="flex flex-col items-center text-center bg-gradient-to-br from-white to-sage-50 p-8 rounded-3xl shadow-lg border border-sage-100 hover:shadow-xl transition-all duration-300 relative overflow-hidden group"
            >
              {/* Premium tag */}
              <div className="absolute top-5 right-5 bg-olive-500 text-white text-xs font-bold px-3 py-1 rounded-full">
                {product.tag}
              </div>

              {/* Product image */}
              <div className="relative w-full h-[420px] mb-6 flex items-center justify-center"> 
                <div className="absolute inset-0 bg-gradient-to-br from-sage-100 to-olive-50 rounded-2xl -z-10 group-hover:from-sage-200 group-hover:to-olive-100 transition-colors"></div>
                <img
                  src={product.image}
                  alt={product.name}
              ...
              <ul className="space-y-2 text-sm text-forest-700 w-full mb-6">
                {product.features.map((feature, i) => (
                  <li key={i} className="flex items-center gap-3 justify-start bg-sage-50 px-3 py-2 rounded-lg group-hover:bg-sage-100 transition-colors">
                    <span className="w-2 h-2 bg-sage-500 rounded-full flex-shrink-0"></span>
                    {feature}
                  </li>
                ))}
        ...
        >
          <Link
            href="/products"
            className="inline-flex items-center bg-gradient-to-r from-sage-600 to-olive-600 text-white px-10 py-4 rounded-xl hover:from-sage-500 hover:to-olive-500 transition-all duration-300 shadow-lg hover:shadow-xl font-semibold text-lg group"
          >
      ...
      {/* Benefits */}
      <section className="py-20 bg-sage-50 relative z-10">
        <div className="max-w-[1920px] mx-auto px-6">
          <h2 className="text-3xl md:text-4xl font-bold text-center mb-12 text-forest-800">
          ...
          <div className="grid md:grid-cols-2 gap-10 items-center">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {benefits.map((benefit, i) => (
                ...
                >
                  <div className="flex items-center justify-center w-14 h-14 rounded-full bg-sage-100">
                    {benefit.icon}
                  </div>
                  <div>
                    ...
                    <p className="text-forest-600 text-sm">{benefit.desc}</p>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <AnimatedSection className="max-w-7xl mx-auto px-4 py-24">
        <motion.h2
          variants={fadeIn}
          className="text-4xl font-bold text-forest-800 mb-16 text-center"
        >
          Our Partners
        </motion.h2>
        <motion.div
